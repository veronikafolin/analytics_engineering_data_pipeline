...
{% macro write_select_groupByColumns_by_vars_from_table(tableName) %}
...
{% macro write_groupBY_groupByColumns_by_vars_from_table(tableName) %}
...
{% set groupBycolumns = var("groupBy") %}
    ...
    {% if groupBycolumns|length > 0 %}
        group by
            {% for col in groupBycolumns %}
            {{tableName}}.{{col}}{% if not loop.last %},{% endif %}
            {% endfor %}
    {% endif %}
